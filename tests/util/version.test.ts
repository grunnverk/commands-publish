...
vi.mock('@grunnverk/git-tools', () => ({ run: vi.fn().mockResolvedValue({ stdout: '1.0.0' }), safeJsonParse: (s: string) => JSON.parse(s) }));
vi.mock('@grunnverk/shared', () => ({ createStorage: () => ({ readFile: vi.fn().mockResolvedValue('{"version":"1.0.0"}'), writeFile: vi.fn() }) }));
